...
for file in [file for file in os.listdir("SOCOFing/Real")]:
    ...
    matches = cv2.FlannBasedMatcher({'algorithm': 1, 'tress': 10}, {}).knnMatch(descriptors_1, descriptors_2, k=2)
    ...
    if len(match_points) / keypoints * 1000 > best_score:
        best_score = len(match_points) / keypoints * 1000
        filename = file 
